...
const STEPS = [
  {
    icon: Menu,
    title: "Open the menu",
    body: "Use the menu to set your city, browse categories, and switch language.",
  },
  {
    icon: MapPin,
    title: "Set your location",
    body: "Pick your city or pin on the map — we sort boutiques by distance.",
  },
  {
    icon: Search,
    title: "Filter outfits",
    body: "Use Categories and Outfits below the header to narrow your search.",
  },
  {
    icon: Store,
    title: "Explore boutiques",
    body: "Scroll to Featured Boutiques or tap Explore Boutiques in the hero.",
  },
  {
    icon: Sparkles,
    title: "Customize your outfit",
    body: "Start a request from the hero, bottom nav, or any boutique profile.",
  },
] as const;
        ...
        <div className="fixed inset-0 z-[120] flex items-end justify-center bg-navy/50 p-4 sm:items-center">
          ...
          >
            <div className="flex items-start justify-between border-b border-border px-5 py-4">
              ...
              <button type="button" onClick={finish} className="rounded-full p-1.5 text-foreground-muted hover:bg-navy/5" aria-label="Close guide">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="px-5 py-6">
              ...
              <div className="mt-5 flex gap-1.5">
                {STEPS.map((_, index) => (
                  <span key={index} className={`h-1.5 flex-1 rounded-full ${index <= step ? "bg-gold" : "bg-navy/10"}`} aria-hidden />
                ))}
              </div>
            </div>
